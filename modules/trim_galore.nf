process TRIM_GALORE {
    ...
    mv !{reads.name}_trimming_report.txt !{meta.id}_trimmed.fq.gz_trimming_report.txt
    sed -i 's/Command line parameters:.*\$/Command line parameters: !{meta.id}_trimmed/g' !{meta.id}_trimmed.fq.gz_trimming_report.txt
